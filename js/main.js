...
function copyScriptCode(btn, code) {
  navigator.clipboard.writeText(code).then(() => {
    const data = getI18nData();
    btn.textContent = data.siteConfig.copiedText;
    showToast(data.siteConfig.copiedText);
    setTimeout(() => {
      btn.textContent = data.siteConfig.copyText;
    }, 2000);
  });
}
    ...
    <section class="hack-section" id="${section.id}" data-index="${index}">
      ...
      <div class="download-action-wrapper reveal">
        ${section.downloads.map(dl => `
          <a href="${dl.link}" class="modern-download-btn" target="_blank">
